...
class TestBernoulli(unittest.TestCase):
    ...
    def test_sum(self):
        ...
        expect = [Nsim * stats.binom(n=5, p=.4).pmf(k) for k in key_list]
...
class TestGeometric(unittest.TestCase):
    ...
    def test_Geometric_to_NBinom(self):
        ...
        for k in key_list:
            outcome = Nsim  * stats.nbinom(n=1,p=0.8).pmf(k)
            if outcome > 5:
                expect.append(outcome / (1-0.8)) 
                new_key.append(k)
        obs = [sims.tabulate()[k] for k in new_key]
        pval = stats.chisquare(obs, expect).pvalue
        self.assertTrue(pval > 0.01)
...
class TestNegativeBinomial(unittest.TestCase):
    ...
    def test_NBinom_Pascal_additive(self):
        ...
        key_list = [keys for keys in sims.tabulate().keys()]
        for k in key_list:
            outcome = Nsim * stats.nbinom(n=10, p=0.6).pmf(k)
            if outcome > 5:
                expect.append(outcome) 
                new_key.append(k)
        obs = [sims.tabulate()[k] for k in new_key]
    ...
    def test_NBinom_to_Geometric(self):
        ...
        key_list = list(sims.tabulate().keys())
        for k in key_list:
            outcome = Nsim * stats.geom(p=0.8).pmf(k)
            if outcome > 5:
                expect.append(outcome)
                new_key.append(k)
        obs = [sims.tabulate()[k] for k in new_key]
...
class TestPoisson(unittest.TestCase):
    ...
    def test_Poisson_additive(self):
        ...
        key_list = list(sims.tabulate().keys())
        for k in key_list:
            outcome = Nsim * stats.poisson(mu=11).pmf(k)
            if outcome > 5:
                expect.append(outcome)
                new_key.append(k)
        obs = [sims.tabulate()[k] for k in new_key]
...
class TestExponential(unittest.TestCase):
    ...
    def test_Exponential_to_Rayleigh(self):
        ...
        cdf = stats.rayleigh(scale=1/math.sqrt(2*5)).cdf
        pval = stats.kstest(sims, cdf).pvalue
        self.assertTrue(pval > .01)
...
class TestLognormal(unittest.TestCase):
    ...
    def test_LogNormal_Product(self):
        ...
        sims = (X*Y).sim(Nsim)
        cdf = stats.lognorm(s=math.sqrt(25+36), scale=exp(21)).cdf
        pval = stats.kstest(sims, cdf).pvalue
        self.assertTrue(pval > .01)
...
class TestRayleigh(unittest.TestCase):
    ...
    def test_Rayleigh_Normal(self):
        ...
        sims =(A**2 + B**2).apply(math.sqrt).sim(Nsim)
